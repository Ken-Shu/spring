...
public class BookDaoImpl implements BookDao{
	...
	@Override
	public Integer getWalledMoney(Integer wid) {
	...
	public Integer updateWallet(Integer wid, Integer money) throws InsufficientAmount{
		//先確認錢包裡的錢
		Integer new_money = getWalledMoney(wid);
		if(new_money <= 0) {
			throw new InsufficientAmount("錢包沒錢了 目前金額 : " + new_money);
		}else if(new_money <= money) {
			throw new InsufficientAmount("目前餘額不足 剩餘金額 : " + new_money);
		}
